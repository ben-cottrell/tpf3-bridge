...
v0.7 fork of the retained v0.3 compiler. The only admitted centreline-intersection
exception is a strictly validated, authored straight fixed diamond. Its named
exclusive resource is additional to, not a replacement for, proximity/track locks.
"""
...
import hashlib
import json
from railgeom.curves import Bezier, flatten, continuous_proximity, join_check
from railgeom.network import RailPath, Network
from railgeom.patterns import Assembly


from railgeom.compiler import Requirement, CompiledRoute, CompiledAssembly
...
def compile_assembly(assembly:Assembly)->CompiledAssembly:
    ...
    for i,a_id in enumerate(ids):
        ...
        for b_id in ids[i+1:]:
            ...
            if not same_component and not shared_port:
                ...
                if near_zero and frozenset((a_id,b_id)) not in declared:
    ...
    for pair, crossing in declared.items():
        res="diamond:"+crossing["id"]
        provenance[res]={"kind":"declared_fixed_diamond_exclusion", **crossing, "rail_connection_created":False}
        for eid in pair: edge_reqs[eid].add(Requirement(res))
    routes={}
    ...
    profile_payload={"compiler":"0.7.0_declared_diamond_fork","source_hash":source_hash,"profile":asdict(p),
    ...
    payload={"network":n.canonical(),"curves":curve_records,"resources":provenance,
       ...
       "profile":asdict(p),"time_model":"external_railops_sectional_motion", "compact_compiler_version":"0.7.0",
       "assessments":{"explicit_topology":"pass","selected_route_G2_continuity":"pass",
          ...
          "real_interlocking_control_table":"unassessed","sectional_release":"external_railops",
